...
tags: [medio, brute force, xss, arbitrary file upload, php, sudo, cron jobs]
...
Como podemos cambiar el valor de este campo, lo que haremos será crear un script, abrir un servidor web poniendo dicho script a disposición, e indicar la URL al script ubicado en nuestra máquina, por si durante la creación o la consulta de la reserva podemos subir dicho script a la máquina víctima.
...
└─$ python3 -m http.server 80
Serving HTTP on 0.0.0.0 port 80 (http://0.0.0.0:80/) ...
172.17.0.2 - - [XX/XXX/XXXX XX:XX:XX] "GET /revshell.php HTTP/1.1" 200 -
```
...
Nos pondremos en escucha con **nc**, ejecutaremos nuestro script y habremos obtenido de esta manera la consola remota:
...
Procedemos a tratar la TTY para poder emplear una consola completamente funcional:
...
Al rato de conectarnos como el usuario `boss`, nos damos cuenta de que se nos cierra la sesión. Esto podría deberse a que hay alguna tarea ejecutándose a intervalos regulares de tiempo que nos cierra la sesión.

Volvemos a obtener acceso como el usuario `www-data`, e buscamos posibles ficheros cron que puedan estar ejecutándose:
...
drwxr-xr-x 1 root root    4096 Jan 15  2025 ..
dr-xrwx--T 1 boss crontab 4096 XXX XX XX:XX crontabs
www-data@dc4ed560f790:/$ ls -la /var/spool/cron/crontabs/
ls: cannot open directory '/var/spool/cron/crontabs/': Permission denied
```

Volvemos a conseguir una consola como el usuario `boss`, y rápidamente comprobamos que dentro del directorio `/var/spool/cron/crontabs/` hay un script (el cual se ejecuta en la tarea cron), y que a su vez ejecuta otros dos scripts:

```bash
boss@dc4ed560f790:/$ ls -la /var/spool/cron/crontabs/
total 16
dr-xrwx--T 1 boss crontab 4096 XXX XX XX:XX .
drwxr-xr-x 1 root root    4096 Jan 15  2025 ..
...
Revisamos el script `/opt/update.sh`, y nos damos cuenta de que es el que está cerrándonos las sesiones:
...
Revisamos también el contenido del script `/tmp/script.sh`, pero vemos que no existe:
